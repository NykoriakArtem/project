...
class TestTagAPIRequests(DriverManager):
    def test_tag_api_requests(self):
        ...
        get = requests.get\
            (f"{settings.URL}/api/ng/b72cd362-9374-40a1-a9b2-f4720b522874/tags/traces/trace/2ASW-K5P0-LPPB-PQ36",
                                headers={
                                    'Authorization': f'{settings.Contrast.AdminUser().auth}',
                                    'API-Key': f'{settings.Contrast.AdminUser().api_key}',
                                    'Content-Type': 'application/json'})
        ...
        delete = requests.delete\
            (f"{settings.URL}/api/ng/b72cd362-9374-40a1-a9b2-f4720b522874/tags/trace/2ASW-K5P0-LPPB-PQ36",
                                headers={
                                    'Authorization': f'{settings.Contrast.AdminUser().auth}',
                                    'API-Key': f'{settings.Contrast.AdminUser().api_key}',
                                    'Content-Type': 'application/json'
                                ...
                                json={"tag": f"{tag_name}"})
        # test will fail on the line 41, status_code should be 200, this is to exercise bug report BR-006.
